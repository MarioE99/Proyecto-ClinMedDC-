...
GO
ALTER TABLE [dbo].[DetalleFactura]  WITH CHECK ADD  CONSTRAINT [FK_DetalleFactura_Medicamento] FOREIGN KEY([idMedicamento])
REFERENCES [dbo].[Medicamentos] ([idMedicamento])
GO
ALTER TABLE [dbo].[DetalleFactura] CHECK CONSTRAINT [FK_DetalleFactura_Medicamento]
GO
...
/****** Object:  StoredProcedure [dbo].[spActualizarDatosDetalle]    Script Date: 2/6/2022 05:53:16 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[spActualizarDatosDetalle]
(@prmIdDetalle int,
@prmPrecioUnidad float)
as
	begin
		update DetalleFactura
		set PrecioUnidad = @prmPrecioUnidad 
	end
GO
...
/****** Object:  StoredProcedure [dbo].[spEliminarDetalle]    Script Date: 2/6/2022 05:53:16 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[spEliminarDetalle]
(@prmIdDetalle int)
AS
	BEGIN
		UPDATE DetalleFactura
		SET estado = 0
		WHERE idDetalle = @prmIdDetalle
	END
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[spListarDetalle]
AS
	BEGIN
		SELECT D.idDetalle
			, D.idFactura
		...
		INNER JOIN Factura ES on D.idFactura = ES.idFactura
		WHERE D.estado = 1 and ES.estado = 1
	END
GO
...
DROP PROCEDURE [spRegistrarDetalle]
...
@prmidFactura INT,
@prmPrecioUnidad float,
@prmCantidadStock INT,
@prmEstado bit
)
AS
	BEGIN

		INSERT INTO DetalleFactura(idDetalle,idFactura,PrecioUnidad,CantidadStock,estado)
		VALUES(@prmidDetalle, @prmidFactura,@prmPrecioUnidad,@prmCantidadStock,1)
		update p set p.Cantidad = p.Cantidad - dv.CantidadStock from Medicamentos p
		INNER JOIN  DetalleFactura dv on dv.idMedicamento = p.idMedicamento
	END
GO
